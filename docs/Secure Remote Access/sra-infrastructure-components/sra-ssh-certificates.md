...
![Illustration for: by way of a Single Sign-on (SSO), the Akeyless Platform connects an SSH client to the server, using your chosen Authentication Methods, while using existing Access Groups and…](https://files.readme.io/d650059-SSH_Certificates.png)
...
>
> It is also possible to enforce host restrictions for SSH connections to only those listed in the SSH Cert Issuer when using [Akeyless Connect](https://docs.akeyless.io/docs/sra-akeyless-connect) by adding the `--secure-access-enforce-hosts-restriction true` flag when creating the Cert Issuer.
