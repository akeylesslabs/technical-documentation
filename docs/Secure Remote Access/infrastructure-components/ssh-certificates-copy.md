...
The command above will upload the existing `RSA` key inside the `/keys/signer/` folder.

Alternatively, you can create a new RSA key:

```shell your-RSA-key-name
akeyless create-dfc-key --name /keys/new-signer/MyRSA --alg RSA2048
```

The command above will create a new `RSA` key inside the `/keys/new-signer/` folder.
