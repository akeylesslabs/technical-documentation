...
- `-g` The base URL of your Akeyless Gateway, the endpoint the CLI uses to authenticate the session and issue the SSH certificate, e.g. `https://<Gateway-EXTERNAL-IP>:8000` retrieved via `kubectl get services` of the service starting with `quick-start-gw`.If your Gateway isn't behind TLS/a load balancer (as in this quick start guide), use `http://` instead of `https://`.
