...
| [HVP](doc:hashicorp-vault-proxy)                                   | 8200  |
...
To set your Gateway default authentication based on [API Key](doc:api-key), provide the relevant `Access ID` and `Access Key` :
...
Set the <code>adminAccessId</code> with your IAM [Authentication Method](doc:access-and-authentication-methods) `Access ID`, where you can define a list of users that will be able to manage your Gateway setting the  `allowedAccessPermissions` field with any other `Access ID` of your  [SAML](doc:saml) ,[OIDC](doc:openid) or an [API Key](doc:api-key) as described [here](https://docs.akeyless.io/docs/gateway-k8s#access-permissions).
...
In both cases, provide your [AWS IAM](doc:aws-iam) Auth Method's `Access ID` as your `adminAccessId`, and at least one other `Access ID` in the `allowedAccessPermissions` section - to provide human users access to [config and manage your Gateway](doc:gateway-k8s#gateway-admins): 
...
And set your [GCP](doc:gcp-auth-method) `Access ID`  as your `adminAccessId` and at least one another `Access ID` in the `allowedAccessPermissions` section, to provide human users access to [config and manage your Gateway](doc:gateway-k8s#gateway-admins): 
...
Set the `adminUIDInitToken` field with your initial root [Universal Identity](doc:universal-identity) token. Set the rotation interval and choose either to generate a child token for your pods using `uidCreateChildTokenPerPod` field. 
