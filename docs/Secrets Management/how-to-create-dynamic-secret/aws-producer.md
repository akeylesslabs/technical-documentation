...
* **Session Tags:**  Key-value pair attributes that you pass when you assume an IAM role or federate a user in AWS STS, can be set using sub-claims, for example - `Key=MyTag,Value={{SomeSubclaimName}}`. relevant only for **Assume Role** mode. 
