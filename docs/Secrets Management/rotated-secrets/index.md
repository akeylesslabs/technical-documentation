...
You can define a rotated secret to automatically update the password at defined intervals, or manually trigger a password update with the CLI or from the Akeyless Console. You can also configure password policy settings (such as length and complexity requirements) for supported rotated secret types. For details, see [CLI Reference - Rotated Secrets](https://docs.akeyless.io/docs/cli-reference-rotated-secrets#create).
...
4. [Retrieve a Rotated Secret Value](https://docs.akeyless.io/docs/retrieve-secret): Get the value of a rotated secret when you need it.
