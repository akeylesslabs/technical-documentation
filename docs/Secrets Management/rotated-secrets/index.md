...
You can define a rotated secret to automatically update the password at defined intervals, or manually trigger a password update with the CLI or from the Akeyless Console. You can also configure password policy settings (such as length and complexity requirements) for supported rotated secret types.
