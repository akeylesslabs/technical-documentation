---
title: GPG Keys
excerpt: GNU Privacy Guard (GPG) Keys
deprecated: false
hidden: false
metadata:
  title: ''
...
GPG stands for GNU Privacy Guard, an implementation of public key cryptography, which can be used both for the more standard operations of encryption keys (encrypt/decrypt), and for message verification via signature. In Akeyless, it is expressed as an additional type for the creation of [Classic Keys](doc:classic-keys), enabling the user to leverage GPG's functionality while keeping it doubly secure under the Akeyless platform.
