...
Note that to create a Classic Key, you must have an active gateway and the **Classic Keys** permission enabled. To set up a gateway, please go to [Akeyless Gateway Overview](doc:api-gw).
...
> 👍 Note
> 
> If you are going to share the Classic Key with a cloud KMS, you need to create a [target](doc:targets) for the key to later be associated with.
...
To associate a Classic Key with a **Cloud KMS** [Target](doc:targets), use this command with the following parameters:

- `target-name`: The name of the [Target](doc:targets) you want to associate with the Classic Key.
...
Different cloud providers demand additional parameters. to see the relevant parameters, go to the correlated page under [External KMS Integration](doc:external-kms).
...
3. Select the relevant Target from the drop-down list, and fill in the required parameters. These parameters may vary between cloud providers. To see the relevant parameters, go to the correlated page under [External KMS Integration](doc:external-kms).
...
3. List the algorithms you’ll allow—usually aes_256_gcm (add aes_128_gcm if you also need it).
...
> • The service account in creds holds RawDecrypt permission (e.g., Cloud KMS CryptoKey Decrypter role).  
