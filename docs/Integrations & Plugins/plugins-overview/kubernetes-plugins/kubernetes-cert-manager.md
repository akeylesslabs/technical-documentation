...
* `role` - `<Access-ID..K8s Auth Config Name>` in Base64-encoded format. Note the Kubernetes Auth config name can be found in the Gateway Console (port `8000`), under the **Kubernetes Auth** menu.
...
At this stage, all the configuration for Kubernetes authentication is set and it is possible to proceed to the next step to [create the certificate request](https://docs.akeyless.io/docs/kubernetes-plugins-plugins-cert-manager#create-a-certificate-request).
