...
Although authorization in Kubernetes is intentionally high-level, you can configure the injector to support full and flexible isolation using Kubernetes policies together with the Akeyless Platform's [Role-Based Access Control (RBAC)](https://docs.akeyless.io/docs/rbac).<br />For details, see [Policy Segregation for Kubernetes](https://docs.akeyless.io/docs/policy-segregation-for-kubernetes).
...
Create an [Access Role](https://docs.akeyless.io/docs/rbac) associate the role with an **Auth Method** and grant access to the secret.<br />For example, the following command creates **K8s_role** role, the role is associated to **K8s_Auth** Auth Method, and grant **read** and **list** access to all the secrets in **K8s** folder
...
### User-Managed Webhook TLS Certificate

By default, the injector chart generates a self-signed CA and serving certificate for its `MutatingWebhookConfiguration` on every helm template render.&#x20;

This is fine for a plain helm `install`/ `helm` upgrade workflow, but it is a problem under **GitOps**: a controller like **ArgoCD** re-renders the chart on every sync cycle, and since the certificate is generated fresh each time, the rendered Secret, the webhook's `caBundle`, and the deployment's checksum/config annotation all drift continuously.&#x20;

With auto-sync and self-heal enabled, this can produce `tls`: bad certificate handshake errors and cause the webhook pods to restart on every sync. It also makes the chart incompatible with cert-manager, since the chart's own generated certificate would overwrite whatever cert-manager issued.

Setting `webhook.tls.existingSecretName` to a non-empty value tells the chart to stop generating a certificate. No Secret is rendered by the chart at all, so a **GitOps** controller has nothing to overwrite, and the render becomes byte-for-byte stable across syncs. No server-side change is required, the injector's webhook server already watches its configured `TLS_SECRET_NAME` and hot-swaps the certificate on rotation without a pod restart.

**Values**

| Value                             | Description                                                                                                                                                                                                                                                                         |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `webhook.tls.existingSecretName`  | Name of a Kubernetes Secret (containing `tls.crt` / `tls.key`) that you manage outside the chart. Setting a non-empty value disables certificate generation and stops the chart from emitting its own Secret. Leaving it unset or empty preserves the default (generated) behavior. |
...
- `existingSecretName` is set but no `caBundle` or `caBundleAnnotations` is provided
