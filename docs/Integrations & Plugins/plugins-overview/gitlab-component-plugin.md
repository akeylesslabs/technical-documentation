...
In this guide, we will use [OAuth 2.0 / JWT](https://docs.akeyless.io/docs/auth-with-oauth-jwt) **Auth Method** to work with the plugin but you can use any [Authentication Method](https://docs.akeyless.io/docs/access-and-authentication-methodss) that is listed in the **Usage** section below.
...
In this example, we will use the `json` mode by setting the `env-file` setting to `false` to fetch [PKI](https://docs.akeyless.io/docs/ssh-and-pkitls-certificates) or [SSH](https://docs.akeyless.io/docs/sra-ssh-certificates#issuing-a-certificate) certificates:
