...
next:
  description: ''
---
**Sectigo** Target enables you to use **Sectigo** as a Public CA with Akeyless [PKI Issuer](doc:ssh-and-pkitls-certificates) .

With Public CA, Akeyless cannot access the private key that signs the certificates. Hence, Akeyless will programmatically contact Sectigo through the [Gateway](doc:api-gw) using the domain owner's account details to validate the certificate signing request.
