...
The AWS IAM principal must be part of the `aws-auth` ConfigMap that is used for authorization to the K8s cluster (learn more about [using the aws-auth ConfigMap](https://docs.aws.amazon.com/eks/latest/userguide/add-user-role.html)). The K8s role to which the AWS IAM principal in the `aws-auth` ConfigMap is bound to will be the same K8s role that tokens generated by the Amazon EKS Dynamic Secret secret will get.
