...
Akeyless supports creating a [PKI Cert Issuer](https://docs.akeyless.io/docs/ssh-and-pkitls-certificates) that deploys an **ACME Server** on the [Gateway](https://docs.akeyless.io/docs/akeyless-gateway), with **EAB** support for secure registration. This setup allows external ACME clients (like [CertBot](https://certbot.eff.org/instructions?ws=other\&os=windows)) to automate the management of TLS certificates within the organization’s security framework.
