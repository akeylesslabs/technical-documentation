...
AI Quorum validate the action against multiple LLM configured for the Gateway under [AI Insights](doc:akeyless-ai-insight), instead of just one. If any model determines the action violates policy, the action is blocked, the system defaults to the most restrictive verdict.
