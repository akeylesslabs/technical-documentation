...
- The Gateway's own Authentication Method has **read** permission on the SAML<br />Authentication Method being configured, so the Gateway can retrieve its configuration to<br />validate sign-ins on its behalf. See [Access Roles](https://docs.akeyless.io/docs/rbac) for permission setup.
