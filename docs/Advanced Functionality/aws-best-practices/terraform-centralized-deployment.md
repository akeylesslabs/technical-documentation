...
Provision **one IAM role per AWS account **, each tied to a **unique External ID generated by Akeyless**. The [Gateway](doc:api-gw) uses an [AWS Target](doc:aws-targets) that contains that account’s **Role ARN** and its matching **External ID**.
