---
title: AWS Roles for Centralized Gateway Using Terraform
excerpt: ''
deprecated: false
hidden: false
metadata:
...
Provision **one IAM role per AWS account**, each tied to a **unique External ID generated by Akeyless**. The [Gateway](https://docs.akeyless.io/docs/akeyless-gateway) uses an [AWS Target](https://docs.akeyless.io/docs/aws-targets) that contains that account’s **Role ARN** and its matching **External ID**.
...
Now, that we have a role in the management account, a role needs to be created in each AWS Account we want to manage from the Akeyless [Gateway](https://docs.akeyless.io/docs/akeyless-gateway).
