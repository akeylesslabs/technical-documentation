...
Provision **one IAM role per AWS account**, each tied to a **unique External ID generated by Akeyless**. The [Gateway](https://docs.akeyless.io/docs/gateway-overview) uses an [AWS Target](https://docs.akeyless.io/docs/cloud-targets) that contains that account’s **Role ARN** and its matching **External ID**.
