...
Provision **one IAM role per AWS account** , each tied to a **unique External ID generated by Akeyless**. The [Gateway](https://docs.akeyless.io/docs/api-gw) uses an [AWS Target](https://docs.akeyless.io/docs/aws-targets) that contains that account’s **Role ARN** and its matching **External ID**.
