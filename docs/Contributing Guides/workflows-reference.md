...
| --- | --- | --- | --- |
| Akeyless GitHub Action Update Jira Task | `.github/workflows/akeyless-github-action-update-jira.yml` | Weekly schedule (`0 2 * * 1`) and manual dispatch | Checks the latest release tag for `akeyless-community/akeyless-github-action` and creates one deduplicated DOCS Jira task only when no task exists yet for that latest tag. It does not inspect technical-documentation repository content. |
