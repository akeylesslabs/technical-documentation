...
| --- | --- | --- | --- |
| Akeyless Public Repos Update Jira Tasks | `.github/workflows/akeyless-public-repos-update-jira.yml` | Weekly schedule (`15 2 * * 1`) and manual dispatch | Checks the latest upstream version signal for every public repository in `akeyless-community` and `akeylesslabs` (excluding `akeylesslabs/technical-documentation`) using a matrix strategy with `fail-fast: false`, job timeout controls, and configurable parallelism. The tracked repository list is sourced from `.github/workflows/data/public-repos-to-track.json`, with base repos in `repos` and optional upstream repos in `third_party_repos`. Manual runs support scope-control inputs (`scope_org`, include and exclude regex, and optional explicit repo list), prerelease policy toggles, API retry count tuning, dry-run mode, soft-fail class configuration, and `include_third_party_repos` to include or exclude third-party tracking. Detection queries `releases/latest` first and falls back to tags with recency-first ordering and optional prerelease filtering. Jira deduplication is keyed by a deterministic repo+tag hash label, and the workflow publishes per-repo result artifacts plus a run summary artifact for triage. It does not inspect technical-documentation repository content. |
