...
| --- | --- | --- | --- |
| Akeyless GitHub Action Update Jira Task | `.github/workflows/akeyless-github-action-update-jira.yml` | Weekly schedule (`0 2 * * 1`) and pull request target on open, reopen, and synchronize events | Creates DOCS Jira tasks for `akeyless-community/akeyless-github-action` updates. On pull request events, it detects version reference updates in diffs and creates one deduplicated triage task. On the weekly schedule, it compares referenced versions to the latest upstream release tag and creates one deduplicated triage task when references are behind. |
