...
| --- | --- | --- | --- |
| Akeyless GitHub Action Update Jira Task | `.github/workflows/akeyless-github-action-update-jira.yml` | Pull request target on open, reopen, and synchronize events | Detects version reference updates to `akeyless-community/akeyless-github-action` in pull request diffs and creates one deduplicated DOCS Jira task with matched references and changed files for documentation triage. |
