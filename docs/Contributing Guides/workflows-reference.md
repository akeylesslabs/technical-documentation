...
| --- | --- | --- | --- |
| Akeyless Public Repos Update Jira Tasks | `.github/workflows/akeyless-public-repos-update-jira.yml` | Weekly schedule (`0 2 * * 1`) and manual dispatch | Checks the latest upstream version signal for every public repository in `akeyless-community` and `akeylesslabs` (excluding `akeylesslabs/technical-documentation`) using a matrix strategy with `fail-fast: false`, job timeout controls, and configurable parallelism. Manual runs support scope-control inputs (`scope_org`, include and exclude regex, and optional explicit repo list), prerelease policy toggles, API retry count tuning, dry-run mode, and soft-fail class configuration. Detection queries `releases/latest` first and falls back to tags with recency-first ordering and optional prerelease filtering. Jira deduplication is keyed by a deterministic repo+tag hash label, and the workflow publishes per-repo result artifacts plus a run summary artifact for triage. It does not inspect technical-documentation repository content. |
