...
| ReadMe Docs Constraints | `.github/workflows/readme-docs-constraints.yml` | Pull requests that change docs Markdown, docs navigation order files, or constraint checker files; manual dispatch | Validates ReadMe compatibility constraints by blocking duplicate Markdown filenames and excessive docs nesting depth in added or renamed docs files, requiring baseline front matter schema keys, and validating impacted `_order.yaml` navigation integrity (required entries and entry targets). Maintains a sticky PR comment that updates on both failing and passing runs with the latest report state. Uses `actions/github-script@v9` for file collection and PR comment updates. |
...
| Secret Scan | `.github/workflows/secret-scan.yml` | Pull requests, push to `v1.0`, and manual dispatch | Runs Gitleaks secret scanning in CI to block committed credentials and sensitive tokens before merge. Performs diff-scoped scanning on PR commit ranges and push events to reduce false positives from historical/baseline findings. Emits job outputs: `scan_summary` (scan completion status), `leaks_found` (count of detected leaks), and `rule_breakdown` (leak categorization by RuleID, for example "generic-api-key: 3, jwt: 2") for integration with automated workflow coordinator reports and Jira issue creation. |
